...
import { buildLoginPath, normalizeLoginRedirectPath } from "@/lib/auth-routing";
...
function getPostSignupDestination(redirectPath = "") {
  ...
  return normalizedRedirect && normalizedRedirect !== "/signup"
    ? normalizedRedirect
    : "/account";
}
...
function SignupPageContent({
  ...
  const handleOAuthMessage = useCallback(
    async (e) => {
      ...
      if (data?.type === "oauth_success") {
        ...
        onClose?.();
        router.push(getPostSignupDestination(redirectPath));
  ...
  return (
    <AuthPageShell
      ...
      title="One secure account for onboarding, care, and follow-up."
      description="Create your HealSend account once, then use it to manage treatment progress, orders, messages, and any next medical step without starting over."
      points={[
        "Save your place across treatment funnels",
        "Track orders, care updates, and next steps",
        "Use Google, Apple, or email sign-up",
      ]}
      secondaryCta={{ href: "/shop", label: "Explore treatments first" }}
      onClose={onClose}
      overlay={overlay}
    >
...
function SignupPageWithSearchParams({
